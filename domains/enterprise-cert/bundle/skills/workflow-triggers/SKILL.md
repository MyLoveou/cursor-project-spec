...
| React/Next 模式、组件架构 | `frontend-patterns` · `react-performance` | P1 |
| Next.js / Turbopack | `nextjs-turbopack` | P1 |
| React Native / Expo / 移动端 | `react-native-patterns` · `@frontend-rn-dev` | P0 |
...
| 加功能 / 改功能 / 修缺陷 / 打磨代码（编排） | `orch-add-feature` / `orch-change-feature` / `orch-fix-defect` / `orch-refine-code` | P1 |
| 从零 MVP | `orch-build-mvp` | P1 |
| ADR、架构决策 | `architecture-decision-records` · `@architect` | P1 |
| 架构审计、六边形架构 | `agent-architecture-audit` · `hexagonal-architecture` | P1 |
| 上线前审计 | `production-audit` | P1 |
...
| 加依赖 | `search-first` | P1 |
| 查文档、API 参考 | `documentation-lookup` | P2 |
| LLM 成本、配额、token | `cost-aware-llm-pipeline` · `cost-tracking` | P2 |
| 上下文压缩、持续学习 | `strategic-compact` · `continuous-learning-v2` | P2 |
...
| 改完 RN（交付前） | `@frontend-rn-dev` 自检 + `react-native-patterns`；可用 `@react-reviewer` 辅助 Hooks |
| 改完 .vue（交付前） | `@vue-reviewer` |
| 上线前 | `production-audit` |
