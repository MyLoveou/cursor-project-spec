...
*For human/AI reference: follow priority 1â†?0 to decide which rule category to focus on first; use `--domain <Domain>` to query details when needed. Scripts do not read this table.*
...
| 7 | Animation | MEDIUM | `ux` | Duration 150â€?00ms, Motion conveys meaning, Spatial continuity | Decorative-only animation, Animating width/height, No reduced-motion |
| 8 | Forms & Feedback | MEDIUM | `ux` | Visible labels, Error near field, Helper text, Progressive disclosure | Placeholder-only label, Errors only at top, Overwhelm upfront |
| 9 | Navigation Patterns | HIGH | `ux` | Predictable back, Bottom nav â‰?, Deep linking | Overloaded nav, Broken back behavior, No deep links |
...
- `focus-states` - Visible focus rings on interactive elements (2â€?px; Apple HIG, MD)
...
- `line-length-control` - Mobile 35â€?0 chars per line; desktop 60â€?5 chars
...
- `weight-hierarchy` - Use font-weight to reinforce hierarchy: Bold headings (600â€?00), Regular body (400), Medium labels (500) (MD)
...
- `duration-timing` - Use 150â€?00ms for micro-interactions; complex transitions â‰?00ms; avoid >500ms (MD)
...
- `exit-faster-than-enter` - Exit animations shorter than enter (~60â€?0% of enter duration) to feel responsive (MD motion)
- `stagger-sequence` - Stagger list/grid item entrance by 30â€?0ms per item; avoid all-at-once or too-slow reveals (MD)
- `shared-element-transition` - Use shared element / hero transitions for visual continuity between screens (MD, HIG)
- `interruptible` - Animations must be interruptible; user tap/gesture cancels in-progress animation immediately (Apple HIG)
- `no-blocking-animation` - Never block user input during an animation; UI must stay interactive (Apple HIG)
- `fade-crossfade` - Use crossfade for content replacement within the same container (MD)
- `scale-feedback` - Subtle scale (0.95â€?.05) on press for tappable cards/buttons; restore on release (HIG, MD)
...
- `disabled-states` - Disabled elements use reduced opacity (0.38â€?.5) + cursor change + semantic attribute (MD)
...
- `touch-friendly-input` - Mobile input height â‰?4px to meet touch target requirements (Apple HIG)
...
- `adaptive-navigation` - Large screens (â‰?024px) prefer sidebar; small screens use bottom/top nav (Material Adaptive)
...
- `touch-target-chart` - Interactive chart elements (points, segments) must have â‰?4pt tap area or expand on touch (Apple HIG)
- `no-pie-overuse` - Avoid pie/donut for >5 categories; switch to bar chart for clarity
- `contrast-data` - Data lines/bars vs background â‰?:1; data text labels â‰?.5:1 (WCAG)
...
- **Stack**: Match the project's framework. The engine ships guidance for many stacks (see [Available Stacks](#available-stacks) below) â€?pass the matching `--stack` (e.g. `nextjs`, `react`, `shadcn`, `vue`, `svelte`, `astro`, `swiftui`, `flutter`, `react-native`).
...
- Run through Quick Reference **Â§1â€“Â?** (CRITICAL + HIGH) as a final review
- Test on 375px (small phone) and landscape orientation
- Verify behavior with **reduced-motion** enabled and **Dynamic Type** at largest size
- Check dark mode contrast independently (don't assume light mode values work)
- Confirm all touch targets â‰?4pt and no content hidden behind safe areas
