...
description: 设计 Agent 驾驭层（Rules/Skills/Agents/Hooks）。改平台配置时使用。
...
- 新增/调整 rules/、skills/、agents/、hooks/ 目录
...
| 层 | 路径 | 职责 |
|----|------|------|
| 硬约束 | project constraints | 不变量、DoD（精简） |
| Rules | rule files (`*.mdc`) | glob / alwaysApply |
| Skills | skill files (`SKILL.md`) | 按需读的流程 |
| Agents | agent files (`*.md`) | `@` 角色 |
| Hooks | hooks configuration | stop 等提醒 |
| 选型 | manifest file | DAILY vs LIBRARY |
